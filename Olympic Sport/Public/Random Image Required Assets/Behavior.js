...
if (!global.scBehaviorSystem) {
  ...
  global.scBehaviorSystem.addCustomTriggerResponse =
    globalTriggerSystem.addListener
  global.scBehaviorSystem.removeCustomTriggerResponse =
    globalTriggerSystem.removeListener
...
function getSign(x) {
  return Math.abs(x) < 0.000001 ? 0 : x > 0 ? 1 : -1
}
...
function safeGetComponent(obj, componentType) {
  return obj.getComponentCount(componentType) > 0
    ? obj.getFirstComponent(componentType)
    : null
}

function getOrAddComponent(obj, componentType) {
  return (
    safeGetComponent(obj, componentType) || obj.createComponent(componentType)
  )
}
...
function whenValueBecomes(
  ...
  var initLastValue = function() {
    lastValue =
      !allowRepeat && optInitialValue === undefined
        ? valueFunc()
        : optInitialValue
  }
  ...
  createAndBindEvent('UpdateEvent', function() {
    ...
    if (
      ...
      (allowRepeat || lastValue !== desiredValue)
    ) {
      callback()
    }
    lastValue = newValue
...
function whenCompareTypeMatches(
  a,
  b,
  compareType,
  callback,
...
function setupTouchEvent() {
  ...
  if (script.touchEventTouchTarget) {
    ...
    var touchComponent = getOrAddComponent(
      targetObj,
...
function setupTurnOnEvent() {
  ...
  global.scBehaviorSystem.addCustomTriggerResponse(
    '_trigger_all_turn_on_behaviors',
    onTrigger
  )
}
  ...
  switch (script.animType) {
    ...
    case 'Image Visual':
      ...
      if (script.animationEndImageVisual.mainPass) {
        script.animationEndImageVisual.mainPass.baseTex.control.setOnFinish(
          onTrigger
        )
      }
      break
    case 'Sprite Visual':
      debugPrint(
        'Sprite Visual is DEPRECATED in Anim Type.\nPlease use Image Visual instead.'
      )
      break
    case 'Animation Mixer':
      ...
      var mixerLayer = script.animationEndAnimMixer.getLayer(
        script.animationEndAnimLayerName
      )
      if (!mixerLayer) {
        debugPrint(
          "Animation Mixer layer couldn't be found: " +
            script.animationEndAnimLayerName
        )
        return
      }
...
function setupTweenEnd() {
  if (!global.tweenManager) {
    debugPrint(
      'Could not find global.tweenManager - have you added Tween Manager to your project?'
    )
    return
  }
  if (!global.tweenManager.isPlaying) {
    debugPrint(
      'global.tweenManager does not contain isPlaying() - is your version up to date?'
    )
    return
  }
  var isPlaying = function() {
    return global.tweenManager.isPlaying(
      script.tweenEndTargetObject,
      script.tweenEndTweenName
    )
  }
...
function setupLookingAt() {
  ...
  whenCompareTypeMatches(
    function() {
      var dir = transformB
        .getWorldPosition()
        .sub(transformA.getWorldPosition())
        .normalize()
      var forward = script.lookingAtFlipForwardVec
        ? transformA.back
        : transformA.forward
      return forward.angleTo(dir)
    },
    cutoffRadians,
    script.lookingAtCompareType,
    onTrigger,
    script.lookingAtAllowRepeat,
    false
  )
}
...
function setupDistanceCheck() {
  ...
  whenCompareTypeMatches(
    function() {
      return transformA
        .getWorldPosition()
        .distance(transformB.getWorldPosition())
    },
    script.distanceCheckDistance,
    script.distanceCheckCompareType,
    onTrigger,
    script.distanceCheckAllowRepeat,
    false
  )
}
...
function setupOnCustomTrigger() {
  ...
  global.scBehaviorSystem.addCustomTriggerResponse(
    script.onCustomTriggerTriggerName,
    onTrigger
  )
}
...
function triggerTextureAnimation() {
  if (
    !(script.animateImageAnimatedTexture || script.animateImageVisualObject)
  ) {
    debugPrint('Image Target must be set!')
    return
  }
  if (script.animateImageAnimatedTexture && script.animateImageVisualObject) {
    script.animateImageVisualObject.mainPass.baseTex =
      script.animateImageAnimatedTexture
  }
  var tex =
    script.animateImageAnimatedTexture ||
    script.animateImageVisualObject.mainPass.baseTex
...
function triggerAnimateMesh() {
  ...
  var mixerLayer = script.animateMeshAnimationMixer.getLayer(
    script.animateMeshLayerName
  )
...
function triggerPlaySound() {
  switch (script.playSoundAction) {
    case 'Play':
      script.playSoundAudioComponent =
        script.playSoundAudioComponent ||
...
function triggerSetRotation() {
  ...
  setTranRot(
    tran,
    quat.fromEulerVec(
      script.setRotationRotationAngle.uniformScale(Math.PI / 180)
    ),
    script.setRotationLocalSpace
  )
}
...
function triggerSetScreenPosition() {
  var screenTran = getFallbackComponent(
    script.setScreenPositionScreenTransform,
...
function triggerSetScreenRotation() {
  var screenTran = getFallbackComponent(
    script.setScreenRotationScreenTransform,
    'Component.ScreenTransform'
  )
  if (!screenTran) {
    debugPrint('Screen Transform must be set!')
  ...
  var rot = quat.fromEulerAngles(
    0,
    0,
    (script.setScreenRotationAngle * Math.PI) / 180
  )
...
function triggerSetScreenSize() {
  var screenTran = getFallbackComponent(
    script.setScreenSizeScreenTransform,
    'Component.ScreenTransform'
  )
  if (!screenTran) {
    debugPrint('Screen Transform must be set!')
...
function triggerSetBlendshapes() {
  ...
  script.setBlendshapesBlendshapes.setBlendShape(
    script.setBlendshapesName,
    script.setBlendshapesWeight
  )
}
...
function triggerInstantiatePrefab() {
  ...
  script.instantiatePrefabPrefab.instantiate(
    script.instantiatePrefabParent || null
  )
}
